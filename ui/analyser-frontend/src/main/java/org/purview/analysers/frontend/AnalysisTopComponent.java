...
final class AnalysisTopComponent extends TopComponent implements Runnable {
    ...
    //Some needed session-related variables:
    private final AnalysisSession session;
    private final ImageMatrix image;
    private final String imageName;

    /**
     * Creates a new AnalysisTopComponent for the given sequence of analysers.
     * @param imageName The name of the image we're analysing. Is used to set the window title.
     * @param matrix The image matrix we should analyse.
     * @param analysers The sequence of analysers to use.
     */
    public AnalysisTopComponent(final String imageName, final ImageMatrix matrix,
        ...
        //Store a new analysis session to work with
    ...
    public void run() {
        ...
        //Create a new analysis stats object, which is used by the analyser
        //session to report back the state of running analysers
        final AnalysisStats stats = new AnalysisStats() {
            ...
            public void reportSubProgress(final float progress) {
                //The analyser-internal "sub-progress" has changed
                final int actualProgress = (int) (1000 * progress);
            ...
            public void reportAnalyser(final String analyser) {
                //There's now another analyser that is running
                SwingUtilities.invokeLater(new Runnable() {
            ...
            public void reportStatus(final String status) {
                //The running analyser has reported a status entry
                SwingUtilities.invokeLater(new Runnable() {
        ...
        //Run the analyser session with our ReportStats
        final scala.collection.Map<Analyser<ImageMatrix>, Set<ReportEntry>> results =
                session.run(stats);

        //Convert the resulting Scala Map to something that we can handle in Java
        ...
        while (analyserIter.hasNext()) {
            ...
            //Do some more Scala conversions
            ...
            //Store the produced report entries
            ...
            //Sort the report entries, first by report level, and then by name
            ...
            //Store the produced java-ized report list
            report.put(analyser, entries);
        }

        //Produce a ResultsTopComponent for displaying our results
        SwingUtilities.invokeLater(new Runnable() {
